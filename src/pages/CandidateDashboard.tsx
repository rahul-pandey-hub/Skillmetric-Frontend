...
const CandidateDashboard = () => {
  ...
  const [selectedExam, setSelectedExam] = useState<CandidateExam | null>(null);
  ...
  const fetchExams = async () => {
    try {
      const response = await api.get('/candidate/exams');
  ...
  const handleStartExam = async (exam: CandidateExam) => {
    try {
      const response = await api.get(`/candidate/exams/${exam._id}/access`);
  ...
  const confirmStartExam = () => {
    ...
    if (selectedExam) {
      navigate(`/candidate/exam/${selectedExam._id}`);
    }
  };
                      ...
                      {exam.attemptStatus === 'completed' ? (
                        <>
                          ...
                          <Button
                            className="w-full"
                            variant="default"
                            onClick={() => navigate(`/candidate/results/${exam._id}`)}
                          >
