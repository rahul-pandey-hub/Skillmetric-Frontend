...
const CandidateExamTaking = () => {
  ...
  const saveCurrentAnswer = async () => {
    ...
    if (answer && examData.sessionId) {
      try {
        await api.post(`/candidate/exams/${examData.sessionId}/save-answer`, {
  ...
  const submitExam = async () => {
    ...
    const currentExamData = examDataRef.current; // Use ref to avoid closure issues
    console.log('📤 currentExamData:', currentExamData);
    console.log('📤 isSubmittingRef.current:', isSubmittingRef.current);

    if (!currentExamData || isSubmittingRef.current) {
      console.log('❌ submitExam() EARLY RETURN:', { examData: !!currentExamData, isSubmitting: isSubmittingRef.current });
      return;
    }

    console.log('📤 Setting isSubmittingRef to true and calling saveCurrentAnswer');
    ...
    try {
      ...
      console.log('📤 Submitting to API...', { examId: currentExamData.exam._id, sessionId: currentExamData.sessionId, answersCount: answersArray.length });

      const response = await api.post(`/candidate/exams/${currentExamData.exam._id}/submit`, {
        sessionId: currentExamData.sessionId,
  ...
  const handleAutoSubmit = useCallback(async (reason: string) => {
    ...
    await submitExam();
  }, [enqueueSnackbar, examData, answers, currentQuestionIndex]);
  ...
  const startExam = async () => {
    try {
      ...
      setExamData(data);
      examDataRef.current = data; // Update ref for socket event handlers
  ...
  const handleForceSubmit = useCallback(async (reason: string) => {
    ...
    if (isSubmittingRef.current) {
      console.log('⚠️ Already submitting, skipping force submit');
      return;
    }
    ...
    try {
      await submitExam();
      console.log('✅ submitExam() completed');
    } catch (error) {
      console.error('❌ submitExam() failed:', error);
    }
  }, [enqueueSnackbar, submitExam]);
  ...
  const setupProctoring = useCallback((data: ExamData) => {
    ...
    socket.current.emit('start-exam', {
      ...
      candidateId: user?.id || null, // null for invitation-based access
    ...
    socket.current.on('force-submit', (submitData: any) => {
      console.log('🚨 FORCE-SUBMIT EVENT RECEIVED:', submitData);
    ...
    const handleVisibilityChange = () => {
      if (document.hidden && examData.sessionId) {
        reportViolation('TAB_SWITCH', {
          description: 'Candidate switched tabs or minimized window',
        });
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
              ...
              <div className="flex items-center justify-end gap-2">
                {warningCount > 0 && (
                  <Badge variant="destructive" className="flex items-center gap-1">
                    ...
                    {examData.proctoringSettings.autoSubmitOnViolation
                      ? `Warnings: ${warningCount}/${examData.proctoringSettings.violationWarningLimit}`
                      : `${warningCount} violation${warningCount !== 1 ? 's' : ''} detected`
                    }
                  </Badge>
                )}
                <Button
        ...
        <Dialog open={showWarningDialog} onOpenChange={setShowWarningDialog}>
          <DialogContent className="max-w-md">
            ...
            <div className="pt-4">
              {/* Extract violation description from backend message (format: "Warning X of Y: Description") */}
              <p className="text-gray-700 mb-4">
                {warningMessage.includes(':') ? warningMessage.split(':').slice(1).join(':').trim() : warningMessage}
              </p>
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                {examData.proctoringSettings.autoSubmitOnViolation ? (
                  ...
                  <p className="text-sm text-red-800">
                    <strong>{warningCount} violation{warningCount !== 1 ? 's' : ''} detected.</strong>
                    {' '}Please avoid such actions during the exam.
                  </p>
                )}
              </div>
            </div>
...
export default CandidateExamTaking;
