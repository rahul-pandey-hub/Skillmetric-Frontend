...
const CandidateResults = () => {
  ...
  const fetchResult = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await api.get(`/candidate/exams/${examId}/result`);
        ...
        <Card className="p-6 mb-6">
          ...
          <div className="border-t pt-4">
            <div className="space-y-4">
              ...
              {result.proctoringReport.violationBreakdown && result.proctoringReport.violationBreakdown.length > 0 ? (
                <div>
                    ...
                    {result.proctoringReport.violationBreakdown.map((violation, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <span className="font-medium capitalize">{violation.type.replace(/_/g, ' ')}</span>
                            <Badge variant={violation.severity === 'HIGH' || violation.severity === 'CRITICAL' ? 'destructive' : 'default'}>
                              {violation.severity}
                            </Badge>
                          </div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <p className="text-sm font-medium text-green-800">
                    ✅ No violations detected during your exam.
                  </p>
                </div>
              )}
